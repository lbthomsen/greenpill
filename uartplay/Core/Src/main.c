...
static inline void process_character(char ch) {
    ...
    default:   // By default, just append the string
            if (strlen < STRING_BUFFER_SIZE) {
                    strncat((char*) &s, &ch, 1); // Danger, Will Robinson!  Could go horribly wrong!
                    ++strlen;
            }
    }
...
void send_string(char *s, int len) {
	HAL_UART_Transmit(&huart2, (uint8_t *) s, len, HAL_MAX_DELAY);
}
...
int main(void)
{
  ...
  uint32_t now = 0, last_blink = 0, last_send = 0;
  char buf[128];
  ...
  while (1)
  {
	  ...
	  if ( now - last_send >= 100) {
		  ...
		  sprintf(buf, "Pretty long buffer to send - tick %lu\n", now);
